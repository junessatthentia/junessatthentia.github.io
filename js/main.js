...
document.addEventListener("DOMContentLoaded", function() {
  ...
  */
  ...
  if(Boolean(togglers)) {
    for (const toggler of togglers) {
      toggler.addEventListener('click', function() {
        const element = this;
        element.classList.toggle('tc-active'); // Toggle class for element
        ...
        target.classList.toggle('tc-active'); // Toggle class for target
      });
    };
  };
  ...
  if(window.screen.width <= 767) {
    // Hide Page Nav
    const pageNav = document.getElementById('page-nav-container');
    // If exist on this page - hide
    Boolean(pageNav) && pageNav.classList.remove('tc-active');
  };
